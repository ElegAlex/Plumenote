...
const roleBadge: Record<string, string> = {
  ...
  dsi: 'bg-blue/10 text-blue',
  public: 'bg-ink-05 text-ink-70',
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-ink">Utilisateurs</h2>
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {users.map((u) => (
            <tr key={u.id} className="border-b border-ink-05 even:bg-bg hover:bg-ink-05">
              <td className="py-3 pr-4 font-medium text-ink">{u.display_name}</td>
              <td className="py-3 pr-4 text-ink-70">{u.username}</td>
              <td className="py-3 pr-4">
                <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${roleBadge[u.role]}`}>
                  {u.role}
                </span>
              </td>
              <td className="py-3 pr-4 text-ink-70">{u.domain_name || '-'}</td>
              <td className="py-3 pr-4 text-ink-45 text-xs">{relativeDate(u.last_login)}</td>
              <td className="py-3 text-right space-x-2">
                <button onClick={() => openEdit(u)} className="text-blue hover:text-blue/80 text-sm">Modifier</button>
                <button onClick={() => resetPassword(u.id)} className="text-amber-600 hover:text-amber-800 text-sm">Reinit. MDP</button>
              </td>
            </tr>
          ))}
          {users.length === 0 && (
            <tr><td colSpan={6} className="py-8 text-center text-ink-45">Aucun utilisateur</td></tr>
          )}
        </tbody>
      </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50" onClick={() => setShowForm(false)}>
          <div className="bg-bg rounded-lg shadow-xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-ink-70 mb-1">Login</label>
                <input
                  type="text"
                  value={form.username}
              ...
              <div>
                <label className="block text-sm font-medium text-ink-70 mb-1">Nom complet</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-ink-70 mb-1">Domaine</label>
                <select
      ...
      {resetResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50">
          <div className="bg-bg rounded-lg shadow-xl w-full max-w-sm p-6">
            <h3 className="text-lg font-semibold text-ink mb-2">Mot de passe temporaire</h3>
            <p className="text-sm text-amber-600 mb-4">Ce mot de passe ne sera plus affiche.</p>
            <div className="flex items-center space-x-2 bg-bg border border-ink-10 rounded-md p-3">
              <code className="flex-1 text-sm font-mono text-ink select-all">{resetResult.password}</code>
              <button
                onClick={copyPassword}
                className="px-3 py-1 text-xs bg-blue text-white rounded hover:bg-blue/90"
              >
                {copied ? 'Copie !' : 'Copier'}
              </button>
            </div>
            <div className="flex justify-end mt-4">
              <button onClick={() => setResetResult(null)} className="px-4 py-2 text-sm text-ink-70 border border-ink-10 rounded-md hover:bg-ink-05">
                Fermer
              </button>
            </div>
          </div>
