...
export default function DomainPage() {
  ...
  if (!domain) {
    return <p className="text-gray-500">Domaine introuvable.</p>
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-500">{domain.doc_count} document{domain.doc_count !== 1 ? 's' : ''}</span>
      </div>

      {docs.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 text-center">
          <p className="text-gray-500 mb-4">Aucun document dans ce domaine. Soyez le premier a contribuer !</p>
          <Link
            to="/documents/new"
          ...
          {docs.map((doc) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-xs text-gray-400">{doc.author_name}</span>
              </div>
              <div className="flex items-center gap-4 flex-shrink-0">
                <span className="text-xs text-gray-400">{doc.view_count} vue{doc.view_count !== 1 ? 's' : ''}</span>
